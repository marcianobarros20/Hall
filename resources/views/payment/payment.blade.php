          ...
          <div class="panel-body">
            <section id="your-trip" class="your-trip">
              <div class="hosting-info">
                ...
                <div class="hide-sm">
                  <p>{{ $result->rooms_address->city }}, {{ $result->rooms_address->state }}, {{ $result->rooms_address->country_name }}</p>
                  <hr>
                  <div class="row-space-1">
                    <strong>{{ $result->room_type_name }}</strong> {{ trans('messages.payments.for') }} <strong>{{ $number_of_guests }} {{ trans_choice('messages.home.guest',$number_of_guests) }}</strong>
                  </div>
                  <div>
  ...
  <table id="billing-table" class="reso-info-table billing-table">
    ...
    <tr class="coupon">
        <td class="name">
          ...
          <span class="coupon-section-link" id="after_apply_coupon" style="{{ (Session::has('coupon_amount')) ? 'display:Block;' : 'display:none;' }}">{{ trans('messages.payments.coupon') }} </span>
          </span>
...
<div class="panel-total-charge">
  ...
  <small><div>
    ...
    <span id="fx-messaging">{{ trans('messages.payments.exchange_rate_booking',['symbol'=>'&euro;']) }} {{ $result->rooms_price->currency->original_symbol }}{{ $price_rate }} {{ $result->rooms_price->currency_code }} ({{ trans('messages.payments.host_listing_currency') }}).</span>
  </div></small>
</div>
                  ...
                  <div class="media-body">
                    <div class="panel panel-quote">
                      <div class="message-to-host control-group">
                        ...
                        <textarea id="message-to-host-input" name="message_to_host" rows="3" class="message-to-host-quote-input" placeholder="{{ trans('messages.payments.message_your_host') }}..."></textarea>
                      </div>
                    </div>
