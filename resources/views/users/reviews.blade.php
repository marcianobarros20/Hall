      ...
      @endfor
       ...
       @if(@$write == 1)
              <li class="media reviews-list-item row-space-2">
                <div class="pull-left text-center">
                  <a class="media-photo media-round" href="{{ url() }}/users/show/{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->id }}">
                    <img width="68" height="68" title="{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->first_name }}" src="{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->profile_picture->src }}" alt="{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->first_name }}">
                  </a>
                <div class="name"><a class="text-muted" href="{{ url() }}/users/show/{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->id }}">{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->full_name }}</a></div>
                </div>
                <div class="media-body">
                  <p>
                    {{ trans('messages.reviews.you_have') }} <b>{{ str_replace('+','',$reviews_to_write[$i]->review_days) }} {{ ($reviews_to_write[$i]->review_days > 1) ? trans_choice('messages.reviews.day',2) : trans_choice('messages.reviews.day',1) }}</b> {{ trans('messages.reviews.to_submit_public_review') }} <a class="text-normal" href="{{ url() }}/users/show/{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->id }}">{{ $reviews_to_write[$i]->review_user(Auth::user()->user()->id)->full_name }}</a>.
                  </p>
      ...
      <div class="panel-body">
          ...
          @for($i=0; $i<$reviews_by_you->count(); $i++)
              <li class="reviews-list-item media row-space-top-2">
                <a class="media-photo media-round pull-left row-space-top-0" href="{{ url() }}/users/show/{{ $reviews_by_you[$i]->user_to }}">
      ...
      <div class="panel-body">
        ...
        <ul class="list-layout reviews-list row-space-top-4">
          @for($i=0; $i<$expired_reviews->count(); $i++)
           ...
           @if(@$expired == 1)
            <li class="media reviews-list-item row-space-top-2">
              <a class="pull-left media-photo media-round" href="{{ url() }}/users/show/{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->id }}">
                <img width="68" height="68" title="{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->first_name }}" src="{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->profile_picture->src }}" alt="{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->first_name }}">
              </a>              
              <div class="media-body response">
                {{ trans('messages.reviews.your_time_to_write_review') }} <a href="{{ url() }}/users/show/{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->id }}">{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->full_name }}</a> {{ trans('messages.reviews.has_expired') }}
                <div>
