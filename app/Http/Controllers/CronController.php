...
class CronController extends Controller
{
    ...
    {
      $reservation_all = Reservation::where('status', '!=', 'Expired')->where('status', '!=', 'Declined')->get();
      ...
      foreach($reservation_all as $row)
      {
        if($row->checkin_cross == 0)
        {
          ...
          $guest_amount = @$guest_referral->if_friend_guest_amount;
          ...
          $host_amount = @$host_referral->if_friend_host_amount;
